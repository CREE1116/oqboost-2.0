...
| ☑   | **Multi-class**                         | 중      | ✅ 네이티브 **joint softmax**(공유 2D-oblique 트리/라운드 + 클래스별 leaf, `multiclass="joint"` 기본) 완료 — split은 부호있는 ΣG 대립쌍 contrast로 1D 축소, 클래스별 log-prior init로 불균형/class_weight 보정. ✅ **OvR**(클래스별 이진 부스터, 행 정규화)도 선택 제공. joint 17-set 평균 acc 0.86 > xgb/lgbm.                                                                                                                            |
...
| ☑   | **Native categorical (target encoding)** | 중~높   | ✅ 완료. `categorical_features` → cross-fitted **target encoding**(empirical-Bayes auto smoothing). high-card서 raw 대비 binary +0.16 AUC·회귀 +0.56 R²·multiclass +0.24 ACC. oblique는 선형결합이라 카테고리에 의미있는 순서 필요 → TE가 제공(axis보다 oblique에 더 유리). **C++ 커널**(make_folds + te_fit_transform, sklearn 의존 0, 100k ~18ms) + 래퍼 제자리치환. binary/회귀 1컬럼, multiclass OvR별 binary 인코딩. sklearn TargetEncoder와 패리티. pickle·warm·explain 지원, feature 레이아웃 불변. |
