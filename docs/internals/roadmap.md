...
| ◐   | **Native categorical (target encoding)** | 중~높   | 무손실 비닝 → ⚠ 1D 제거 후 퇴화. **재측정으로 판정 뒤집힘**(2026-06-24): high-card+cat×cont서 TE +0.162 vs lossless +0.008(20배). oblique는 선형결합이라 카테고리에 의미있는 1D 순서 필요 → TE가 제공(axis보다 oblique에 더 유리). **설계: `categorical_features` → target encoding.** 엔진=sklearn `TargetEncoder`(1.3+, cross-fitted·smoothing·unseen·multiclass 내장), wrapper-level·C++무변경. 미정: multiclass 컬럼팽창(OvR별 binary 인코딩 1컬럼 선호). 프로토타입 단계. |
