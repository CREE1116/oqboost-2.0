...
- **사선의 순기여**: depth 효율("얕은 사선 = 깊은 축") — Lemma 1의 "동등" + 복잡도 payoff를 실측으로 뒷받침. 사선은 우월한 함수족이 아니라 **효율적 표현**.
...
**한 줄 결론.** OQBoost는 "대부분의 정형 데이터는 저차(≤2-way) 상호작용의 가산적 형태"라는 귀납적 편향을 가진 모델이며, ① 그 편향이 실데이터에서 대체로 참이고($\epsilon$ 작음), ② 사선은 그 구조를 **더 적은 복잡도로**(depth 효율 → $\sqrt{\log D}$) 표현하며, ③ 편향이 깨지는 고차 셋에서는 LOB가 처방이 된다.
